...
{
    NSDate* currentDate = [NSDate currentData];
    if (([[self yearString] integerValue] == [[currentDate yearString] integerValue])
        && ([[self monthString] integerValue] == [[currentDate monthString] integerValue])
        && ([[self dayString] integerValue] == [[currentDate dayString] integerValue])) {
        return [self stringTime];
    }
    
    if ([[self yearString] integerValue] == [[currentDate yearString] integerValue]) {
        return [self stringYearMonthDayHourMinute];
    }
    else
    {
        return [NSDate stringMonthDayHourMiniteWithDate:self];
    }
}
...
{
    long long currentTimes = [NSDate currentTimeNumber];
    NSString *hhmm = [NSDate dateStringFromDate:self dateFormat:[NSDate dateFormatHHmm]];
    NSString *str;
    NSInteger chaDay = [self daysBetweenCurrentDateAndDate];
    if (chaDay == 0) {
        if ((currentTimes-[NSDate currentTimeNumberByDate:self]) < 3*60) {
            str = @"刚刚";
        }else{
            str = [NSString stringWithFormat:@"%@", hhmm];
        }
    }else if (chaDay == 1){
        str = [NSString stringWithFormat:@"明天 %@", hhmm];
    }else if (chaDay == -1){
        str = @"昨天";
    }else{
        str = [NSDate dateStringFromDate:self dateFormat:dateFormat];
    }
...
{
    return [NSDate dateWithTimeIntervalSince1970:[unixtime longLongValue]];
}
...
+ (NSDate*)currentData
{
    return [NSDate dateWithTimeIntervalSince1970:[NSDate currentTimeNumber]];
}

+ (NSString*)currentTimeString
{
    return [NSString stringWithFormat:@"%lld", [NSDate currentTimeNumber]];
}
